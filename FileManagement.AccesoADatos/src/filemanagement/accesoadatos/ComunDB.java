...
// Clase que utilizaremos para obtener la conexion a la base de datos, para insertar, modificar, eliminar, obtener  de datos de nuestras tablas
public class ComunDB {

    // Documentacion de clases-anidadas -->
    // https://javadesdecero.es/poo/clases-anidadas/
    class TipoDB { // La clase TipoDB se utilizara para saber que tipo de gestor de base de datos
                   // estamos accediendo
    ...
    /*
     * El metodo obtenerConexion() nos va a devolver una conexion abierta al gestor
    ...
    /*
     * El metodo createStatement lo utilizaremos para devolver un Statement el cual
     * permite
     * ejecutar una consulta de INSERT, UPDATE, DELETE , SELECT en la base de datos
     * aprender mas sobre la clase Statement -->
     * https://www.ibm.com/docs/es/i/7.3?topic=types-statement-objects
     */
    ...
    /*
     * El metodo createPreparedStatement lo utilizaremos para devolver un
     * PreparedStatement el cual permite
     * ejecutar una consulta de INSERT, UPDATE, DELETE , SELECT en la base de datos
     * y
     * nos permite utilizar parametros en la consultas para evitar la inyección sql
     * aprender mas sobre la clase PreparedStatement -->
     ...
     * sirver para almacenar el resultado de la consulta SELECT -->
    ...
    /*
     * El metodo ejecutarSQL permite ejecutar una consulta INSERT, UPDATE, DELETE
     * directamente a la base de datos
     */
    public static int ejecutarSQL(String pSql) throws SQLException {
        int result;
        try (Connection connection = obtenerConexion();) { // Obtener la conexion y encerrarla en try para cierre
                                                           // automatico
            Statement statement = connection.createStatement();
            result = statement.executeUpdate(pSql); // Ejecutar la consulta SQL y devolvera un valor entero con la
    ...
    /*
     * La clase UtilQuery la utilizaremos para concantenar mejor los filtros de la
     * consultas SELECT
     * a la base de datos
     */
    class UtilQuery {
        ...
        private PreparedStatement statement; // Propiedad para almacenar PreparedStatement al cual se le agregaran los
                                             // parametros ?
        private int numWhere; // Propiedad para almacenar la cantidad de campos incluidos en el WHERE de una
                              // consulta SELECT
        ...
        // Fin de los metodos del encapsulamiento de las propiedades SQL,statement,
        // numWhere

        /*
         * El metodo AgregarWhereAnd lo utilizaremos para cuando agreguemos un nuevo
         * campo en la consulta SELECT y automaticamente
         * agregar el WHERE o AND segun la cantidad de campos agregados en la consulta,
         * tambien incrementara en 1 la cantidad de campos
         * en el WHERE
         */
        public void AgregarWhereAnd(String pSql) {
            if (this.SQL != null) {
                if (this.numWhere == 0) {
                    // Si el numWhere es cero significa que es el primer campo agregado en la
                    // consulta SELECT entonces agregamos el WHERE
                    this.SQL += " WHERE ";
                } else {
                    // Si el numWhere es diferente a cero significa que agregaremos el AND a la
                    // consulta SELECT
                    this.SQL += " AND ";
                }
                this.SQL += pSql; // Agregar el valor extra a la consulta
            }
            this.numWhere++; // Incrementar en 1 la cantidad de campos actuales en el WHERE de la consulta
                             // SELECT
        }
    }
}
